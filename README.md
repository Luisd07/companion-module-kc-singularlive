...
- **Polling default 2s → 30s, idle default 15s → 120s.** Each poll is one REST
  API call per app, and 2s with 3 apps (~130k/day) exceeds Singular's daily REST
  quota. The idle interval only applies when it is slower than the polling
  interval, so its default moves up to stay meaningful. The setting's tooltip
  now shows the cost; use 0 if only Companion controls the apps.
