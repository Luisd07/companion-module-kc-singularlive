...
export default class SingularLive {
	...
	getModelStates() {
		return fetch(url + this.token + '/model', this.GETOption())
			.then((res) => res.json())
			.then((result) => {
				const states = {}
				const subcomps = result[0]?.subcompositions ?? []
	...
	updatePayload(composition, payload) {
		if (!composition || !payload || typeof payload !== 'object') return
	...
	pressButtons(controlnodes) {
		if (!Array.isArray(controlnodes) || controlnodes.length === 0) return
